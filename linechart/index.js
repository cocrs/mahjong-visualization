var dataset = { "和了率": [], "平均和了点": [], "和了点期待値": [] };

// data
d3.json("./agariJunme.json").then(function (json) {
    for (j in json) {
        for (d in dataset) {
            dataset[d].push({ x_value: json[j]['巡目'], y_value: json[j][d] });
        }
    }
    update_chart("和了率");
});
...
function update_chart(s) {
    svg.call(hover, dataset[s], false);
    update(dataset[s]);
    update_y_label(s);
    setTimeout(() => {  svg.call(hover, dataset[s], true); }, 2000);
}